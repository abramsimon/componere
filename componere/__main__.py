...
Usage:
    ...
    componere --i=<input> --o=<output> (--detail | --overview=<level> | --area=<area-name> |  --areas | --all | --wiki)...
    componere -h | --help
...
def create_wiki(areas, components, levels, teams):
    ...
    for component in sorted(components):
        ...
        if obj.type_identifier is not None:
            comp_file.write("* Type: {0}\n".format(obj.type_identifier))
...
def _main(argv=sys.argv[1:]):
    ...
    load_input.load_objects(
        input_path, areas, components, levels, teams, types)
